window.OT = {
    ...
    recognizeFace: function (streamId, isPublisher, sucess, error) {
        return Cordova.exec(sucess, error, OTPlugin, 'recognizeFace', [streamId, isPublisher]);
    },
    getHelper: function () {
...
getPosition = function (divName) {
    ...
    var rect = pubDiv.getBoundingClientRect();
    return {
        width: rect.width,
        height: rect.height,
        top: rect.top,
...
replaceWithVideoStream = function (divName, streamId, properties) {
    ...
    element.setAttribute('class', element.className + ' ' + 'OT_root ' + typeClass);
    element.setAttribute('data-streamid', streamId);
    if (properties.top)
        element.style.top = properties.top + 'px';
    if (properties.left)
        element.style.left = properties.left + 'px';
...
TBUpdateObjects = function () {
    ...
    for (_i = 0, _len = objects.length; _i < _len; _i++) {
        ...
        var isPublisher = (id === 'video-publisher');
        var stream = null;
        if(isPublisher && OT.session.publisher)
        {
            stream = OT.session.publisher.stream;
        }else{
...
TBGetScreenRatios = function () {
    var ratioMultiplier = window.devicePixelRatio;
    return {
        widthRatio: ratioMultiplier,
        heightRatio: ratioMultiplier
    };
};
...
TBPublisher = (function () {
    function TBPublisher(targetElement, properties, completionHandler) {
        ...
        var borderRadius, cameraName, height, name, obj, onError, onSuccess, position, publishAudio, publishVideo, ratios, width, zIndex, clientLeft, clientTop, _ref, _ref1, _ref2, _ref3, _ref4, _ref5;
        ...
        if (this.properties != null) {
            ...
            clientTop = (_ref4 = this.properties.top) != null & '' ? _ref4 : position.top;
            clientLeft = (_ref5 = this.properties.left) != null & '' ? _ref5 : position.left;
    ...
    TBPublisher.prototype.streamCreated = function (event) {
        ...
        pdebug('publisher streamCreatedHandler', this.session.connection);
        this.stream = new TBStream(event.stream, this.session.connection);
    ...
    TBPublisher.prototype.publishMedia = function (media, state) {
        ...
        if (media === 'publishAudio') {
            this.stream.hasAudio = state;
        } else if (media === 'publishVideo') {
            this.stream.hasVideo = state;
        }
