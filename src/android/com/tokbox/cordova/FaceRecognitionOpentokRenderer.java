...
public class FaceRecognitionOpentokRenderer extends BaseVideoRenderer
{
    ...
    boolean mShouldRecognizeFaceOnNextFrame, mIsPublisher;
    ...
    static class MyRenderer implements GLSurfaceView.Renderer
	{
        ...
        private FaceRecognitionOpentokRenderer mCustomVideoRenderer;
		static private FaceDetector fdetector;

        public MyRenderer(FaceRecognitionOpentokRenderer parent)
		{
            this.mCustomVideoRenderer = parent;

			if(fdetector == null)
			{
				fdetector = new FaceDetector.Builder(mCustomVideoRenderer.mContext)
        ...
        public void displayFrame(Frame frame) throws JSONException
		{
            mFrameLock.lock();
            ...
            if(mCustomVideoRenderer.mShouldRecognizeFaceOnNextFrame)
			{
				synchronized(fdetector)
				{
					mCustomVideoRenderer.mShouldRecognizeFaceOnNextFrame = false;
					...
					if(fdetector.isOperational())
					{
						...
						if( faces.size() > 0 )
						{
							...
							if(face != null)
							{
								...
								{
									if(mCustomVideoRenderer.mIsPublisher)
									{
									...
									resultdict.put("streamId", mCustomVideoRenderer.mSid);

									mCustomVideoRenderer.mCallbackContext.sendPluginResult(new PluginResult(PluginResult.Status.OK, resultdict));
								}
								else
								{
									JSONObject resultdict = new JSONObject();
									...
									mCustomVideoRenderer.mCallbackContext.sendPluginResult(new PluginResult(PluginResult.Status.ERROR, resultdict));					
								}
							}
							else
							{
    ...
    public FaceRecognitionOpentokRenderer(Context context) {
	...
	public void setToRecognizeFace(CallbackContext callbackContext, String sid, boolean isPublisher){
        mShouldRecognizeFaceOnNextFrame = true;
		mCallbackContext = callbackContext;
		mSid = sid;
		mIsPublisher = isPublisher;
    }
}